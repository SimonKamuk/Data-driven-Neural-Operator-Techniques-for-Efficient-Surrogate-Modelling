...
title = @sprintf "Example DeepONet input/output. MSE %.2e" Flux.mse(unseen_deepo_vals, unseen_v_vals .+ 1)
...
unseen_deepo_vals = reshape(model(reshape(y_locs,1,:), unseen_u_vals),:)
unseen_deepo_deriv = ForwardDiff.derivative.(y->model([y], unseen_u_vals)[], x_locs)
title = @sprintf "Example DeepONet input/output. MSE %.2e" Flux.mse(unseen_deepo_vals, unseen_v_vals)
